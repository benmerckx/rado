...
test('insert: placeholders on columns with encoder', async ctx => {
  ...
  expect(result).toEqual([{id: 1, jsonb: ['foo', 'bar']}])
})
...
test('select from a many subquery', async ctx => {
  ...
  const _typeCheck: {population: number; name: string}[] = res
...
test('select from a one subquery', async ctx => {
  ...
  const _typeCheck: {cityName: string; name: string}[] = res
...
test('select a field without joining its table', ctx => {
  ...
  expect(() =>
    db.select({name: users2Table.name}).from(usersTable).prepare('query')
  ).toThrowError()
})
...
test('select count w/ custom mapper', async ctx => {
  ...
  function count(value: Column | SQLWrapper): SQL<number>
  function count(value: Column | SQLWrapper, alias: string): SQL.Aliased<number>
  function count(
    value: Column | SQLWrapper,
    alias?: string
...
test('materialized view', async ctx => {
  ...
  const newYorkers1 = pgMaterializedView('new_yorkers').as(
    db.select().from(users2Table).where(eq(users2Table.cityId, 1))
  )
...
test('select from existing view', async ctx => {
  ...
  const newYorkers = schema
    .view('new_yorkers', {
      id: integer('id').notNull()
    })
    .as(sql`select id from ${usersTable}`)

  await db.drop(schema)
  await db.create(schema)
  await db.execute(
    sql`create view ${newYorkers} as select id from ${usersTable}`
  )
  ...
  const result = await db
    ...
    .from(usersTable)
    .innerJoin(newYorkers, eq(newYorkers.id, usersTable.id))
...
test('orderBy with aliased column', ctx => {
  ...
  const test = sql`something`.as('test')
  const query = db
    .select({
      test
    })
...
test('timestamp timezone', async ctx => {
  ...
  const usersTableWithAndWithoutTimezone = pgTable(
  ...
  await db
    ...
    .values({name: 'With default times'})
  ...
  expect(Math.abs(users[0]!.updatedAt.getTime() - Date.now())).toBeLessThan(
    2000
  )
  ...
  expect(Math.abs(users[1]!.updatedAt.getTime() - date.getTime())).toBeLessThan(
    2000
  )
  expect(Math.abs(users[1]!.createdAt.getTime() - date.getTime())).toBeLessThan(
    2000
  )
})
...
test('test if method with sql operators', async ctx => {
  ...
  const result3 = await db
    .select()
    .from(users)
    .where(or(eq(users.id, 1).if(condition3), eq(users.id, 2).if(condition3)))
  ...
  const result6 = await db
    .select()
    .from(users)
    .where(or(eq(users.id, 1).if(condition6), eq(users.id, 2).if(condition6)))
  ...
  expect(result9).toEqual([{id: 2, name: 'Alice', age: 21, city: 'New York'}])
  ...
  const result10 = await db
    ...
    .where(
      and(
        sql<boolean>`length(${users.name}) <= ${condition10.term1}`.if(
          condition10.term1
        ),
  ...
  const result13 = await db
    .select()
    .from(users)
    .where(sql<boolean>`(city = 'New York' or age >= 22)`.if(condition13))
  ...
  const result14 = await db
    .select()
    .from(users)
    .where(sql<boolean>`(city = 'London' and age >= 23)`.if(condition14))
...
test('mySchema :: build query insert with onConflict do update / multiple columns', async ctx => {
  ...
  const query = db
    ...
    .values({name: 'John', jsonb: ['foo', 'bar']})
    .onConflictDoUpdate({
      target: [usersMySchemaTable.id, usersMySchemaTable.name],
      set: {name: 'John1'}
    })
    .toSQL()
...
test('proper json and jsonb handling', async ctx => {
  ...
  await db
    .insert(jsonTable)
    .values({json: {name: 'Tom', age: 75}, jsonb: {name: 'Pete', age: 23}})
...
test('update ... from with join', async ctx => {
  ...
  await db.insert(states).values([{name: 'New York'}, {name: 'Washington'}])
  await db
    .insert(cities)
    .values([
...
test('all types', async ctx => {
  ...
  type A = Pick<(typeof rawRes)[0], 'arrbool'>
