...
export async function testInclude(db: Database, test: DefineTest) {
  ...
  const posts = include(db.select().from(Post).where(eq(Post.userId, User.id)))
  ...
  const postsWithUser = await db
    .select({
      ...Post,
      user: include.one(db.select().from(User).where(eq(User.id, Post.userId)))
    })
    .from(Post)
